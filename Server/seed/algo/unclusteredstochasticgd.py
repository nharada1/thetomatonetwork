...
#        P_tau = n x tau array of performance values
...
#		the performance determined by N[:,t-1] (N_tau[:,tau-1]) is at P[:,t-1] (P_tau[:,tau-1])
...
# 3) g <- slope of linear regression on locus (N[i,s],P[i,s]) := (N_regress,P_regress)
...
def calcNutrientUpdate(N_tau,P_tau,L,T):
	...
	for i in range(0,n):
		...
		P_regress[i_regress] = P_tau[i,tau-1]
	...
	if abs(g) > L:
		eta = 10/(math.sqrt(T*abs(g)))
	else:
		eta = 10/(math.sqrt(T*L))
